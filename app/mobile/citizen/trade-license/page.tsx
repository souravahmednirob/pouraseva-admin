...
export default function TradeLicensePage() {
  ...
  const [form, setForm] = useState({ bizName: "", bizType: "", bizAddress: "", ward: "", ownerName: "", nid: "", mobile: "", email: "", paymentMethod: "" });
  const [showSuccess, setShowSuccess] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { t } = useLang();
  const steps = [t("ব্যবসার তথ্য", "Business Info"), t("মালিকের তথ্য", "Owner Info"), t("পর্যালোচনা ও পেমেন্ট", "Review & Payment")];
          ...
          <div className="absolute inset-0 z-50 bg-[#F0F7FF] dark:bg-[#0F172A] flex flex-col">
            <div className="relative overflow-hidden">
              ...
              <div className="relative flex flex-col items-center pt-16 pb-10">
                ...
                <p className="text-green-100 text-sm font-bangla">{t("ট্রেড লাইসেন্সের আবেদন গ্রহণ করা হয়েছে", "Trade license application received")}</p>
              </div>
            </div>
            <div className="flex-1 px-6 -mt-3">
              <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-gray-700/50 shadow-lg rounded-2xl p-5 mb-5">
                ...
                <p className="text-3xl font-mono text-electric-blue font-bold text-center tracking-wider mb-3">TL-2026-00421</p>
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative px-5 pt-4 pb-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div><h1 className="text-lg font-bold text-white font-bangla">{t("ট্রেড লাইসেন্স আবেদন", "Trade License")}</h1><p className="text-white/70 text-xs font-bangla">{t("ফি: ৳৩,০০০", "Fee: ৳3,000")}</p></div>
            </div>
            <div className="flex items-center gap-1 mt-2">
              {steps.map((s, i) => (<div key={i} className="flex items-center flex-1 last:flex-none"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ${i + 1 < step ? "bg-white text-violet-600" : i + 1 === step ? "border-2 border-white text-white" : "bg-white/20 text-white/50"}`}>{i + 1 < step ? <CheckCircle className="w-4 h-4" /> : i + 1}</div>{i < steps.length - 1 && <div className={`flex-1 h-0.5 mx-1 ${i + 1 < step ? "bg-white" : "bg-white/20"}`} />}</div>))}
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {step === 1 && (
            <div className="space-y-3.5">
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("ব্যবসার নাম", "Business Name")}</span><input type="text" className={ic} value={form.bizName} onChange={(e) => set("bizName", e.target.value)} placeholder="যেমন: রহিম স্টোর" /></label>
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("ব্যবসার ধরন", "Business Type")}</span><select className={ic} value={form.bizType} onChange={(e) => set("bizType", e.target.value)}><option value="">নির্বাচন করুন</option><option>{t("খুচরা", "Retail")}</option><option>{t("পাইকারি", "Wholesale")}</option><option>{t("সেবা", "Service")}</option><option>{t("উৎপাদন", "Manufacturing")}</option><option>{t("খাদ্য ও রেস্তোরাঁ", "Food & Restaurant")}</option><option>অন্যান্য</option></select></label>
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("ব্যবসার ঠিকানা", "Business Address")}</span><input type="text" className={ic} value={form.bizAddress} onChange={(e) => set("bizAddress", e.target.value)} placeholder="দোকান/অফিসের সম্পূর্ণ ঠিকানা" /></label>
              ...
              <button onClick={() => setStep(2)} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-white font-bangla font-bold flex items-center justify-center gap-2 shadow-lg mt-2">{t("পরবর্তী", "Next")} <ArrowRight className="w-4 h-4" /></button>
            </div>
          )}
          {step === 2 && (
            <div className="space-y-3.5">
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("মালিকের নাম", "Owner Name")}</span><input type="text" className={ic} value={form.ownerName} onChange={(e) => set("ownerName", e.target.value)} placeholder="পূর্ণ নাম বাংলায়" /></label>
              ...
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("মোবাইল নম্বর", "Mobile")}</span><input type="tel" className={ic} value={form.mobile} onChange={(e) => set("mobile", e.target.value)} placeholder="০১XXXXXXXXX" /></label>
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("ইমেইল (ঐচ্ছিক)", "Email (Optional)")}</span><input type="email" className={ic} value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="example@email.com" /></label>
              <div className="flex gap-3 mt-2">
                ...
                <button onClick={() => setStep(3)} className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-white font-bangla font-bold flex items-center justify-center gap-2">{t("পরবর্তী", "Next")} <ArrowRight className="w-4 h-4" /></button>
              </div>
            </div>
          )}
          {step === 3 && (
            <div className="space-y-4">
              <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-gray-700/50 rounded-2xl p-4 space-y-2.5">
                ...
                {[["ব্যবসার নাম", form.bizName || "রহিম স্টোর"], ["ব্যবসার ধরন", form.bizType || "খুচরা"], ["মালিক", form.ownerName || "রহিম উদ্দিন"], ["ওয়ার্ড", `ওয়ার্ড ${form.ward || "৩"}`]].map(([l, v]) => (
              ...
              <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-gray-700/50 rounded-2xl p-4 flex items-center justify-between"><span className="text-gray-500 font-bangla">{t("ফি", "Fee")}</span><span className="text-2xl font-bold text-electric-blue">৳৩,০০০</span></div>
