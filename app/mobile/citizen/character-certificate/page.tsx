...
export default function CharacterCertificatePage() {
  ...
  const [form, setForm] = useState({ nid: "", name: "", fatherName: "", profession: "", address: "", ward: "", purpose: "", submitTo: "", paymentMethod: "" });
  const [showSuccess, setShowSuccess] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { t } = useLang();
  const steps = [t("ব্যক্তিগত তথ্য", "Personal Info"), t("সনদের উদ্দেশ্য", "Purpose"), t("পর্যালোচনা ও পেমেন্ট", "Review & Payment")];
          ...
          <div className="absolute inset-0 z-50 bg-[#F0F7FF] dark:bg-[#0F172A] flex flex-col">
            <div className="relative overflow-hidden">
              ...
              <div className="relative flex flex-col items-center pt-16 pb-10">
                ...
                <p className="text-green-100 text-sm font-bangla">{t("চারিত্রিক সনদের আবেদন গ্রহণ করা হয়েছে", "Character certificate application received")}</p>
              </div>
            </div>
            <div className="flex-1 px-6 -mt-3">
              <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-gray-700/50 shadow-lg rounded-2xl p-5 mb-5">
                ...
                <p className="text-3xl font-mono text-electric-blue font-bold text-center tracking-wider mb-3">CHC-2026-00087</p>
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative px-5 pt-4 pb-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div><h1 className="text-lg font-bold text-white font-bangla">{t("চারিত্রিক সনদ আবেদন", "Character Certificate")}</h1><p className="text-white/70 text-xs font-bangla">{t("ফি: ৳৫০", "Fee: ৳50")}</p></div>
            </div>
            <div className="flex items-center gap-1 mt-2">
              {steps.map((s, i) => (<div key={i} className="flex items-center flex-1 last:flex-none"><div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ${i + 1 < step ? "bg-white text-teal-600" : i + 1 === step ? "border-2 border-white text-white" : "bg-white/20 text-white/50"}`}>{i + 1 < step ? <CheckCircle className="w-4 h-4" /> : i + 1}</div>{i < steps.length - 1 && <div className={`flex-1 h-0.5 mx-1 ${i + 1 < step ? "bg-white" : "bg-white/20"}`} />}</div>))}
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {step === 1 && (
            <div className="space-y-3.5">
              ...
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("পেশা", "Profession")}</span><input type="text" className={ic} value={form.profession} onChange={(e) => set("profession", e.target.value)} placeholder="যেমন: ব্যবসায়ী, চাকরিজীবী" /></label>
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("ঠিকানা", "Address")}</span><input type="text" className={ic} value={form.address} onChange={(e) => set("address", e.target.value)} placeholder="সম্পূর্ণ ঠিকানা" /></label>
              ...
              <button onClick={() => setStep(2)} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-teal-600 to-teal-500 text-white font-bangla font-bold flex items-center justify-center gap-2 shadow-lg mt-2">{t("পরবর্তী", "Next")} <ArrowRight className="w-4 h-4" /></button>
            </div>
          )}
          {step === 2 && (
            <div className="space-y-3.5">
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("সনদের উদ্দেশ্য", "Purpose")}</span><select className={ic} value={form.purpose} onChange={(e) => set("purpose", e.target.value)}><option value="">নির্বাচন করুন</option><option>{t("চাকরি", "Employment")}</option><option>{t("বিদেশ যাত্রা", "Foreign Travel")}</option><option>{t("শিক্ষা প্রতিষ্ঠান", "Education")}</option><option>অন্যান্য</option></select></label>
              <label className="block"><span className="text-xs text-gray-500 dark:text-gray-400 font-bangla mb-1.5 block font-medium">{t("কোথায় জমা দিবেন", "Submit To")}</span><input type="text" className={ic} value={form.submitTo} onChange={(e) => set("submitTo", e.target.value)} placeholder="যেমন: ABC কোম্পানি, পাসপোর্ট অফিস" /></label>
              <div className="flex gap-3 mt-2">
                ...
                <button onClick={() => setStep(3)} className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-teal-600 to-teal-500 text-white font-bangla font-bold flex items-center justify-center gap-2">{t("পরবর্তী", "Next")} <ArrowRight className="w-4 h-4" /></button>
              </div>
            </div>
          )}
          {step === 3 && (
            <div className="space-y-4">
              <div className="bg-white dark:bg-[#1E293B] border border-gray-100 dark:border-gray-700/50 rounded-2xl p-4 space-y-2.5">
                ...
                {[["নাম", form.name || "রহিম উদ্দিন"], ["পেশা", form.profession || "ব্যবসায়ী"], ["উদ্দেশ্য", form.purpose || "চাকরি"], ["জমা দিবেন", form.submitTo || "ABC কোম্পানি"]].map(([l, v]) => (
